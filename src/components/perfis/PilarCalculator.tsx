    ...
    const handleAnalyzeWithAI = async () => {
        ...
        try {
            const response = await fetch("/api/ai-compare", {
                ...
                body: JSON.stringify({
                    analysisType: "pilar-estrutural",
                    context: {
                        inputs: { height, additionalHeight, axialLoad, steelType, safetyFactor },
                        result,
                        supportReactions,
                ...
                {result && (
                     <Card className="mt-4 bg-primary/5 border-primary/20">
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <RefinedButton type="button" onClick={handleAnalyzeWithAI} className="w-full" disabled={isAiAnalyzing}>
                                {isAiAnalyzing ? <><Loader className="animate-spin mr-2"/> Consultando IA...</> : <><Sparkles className="mr-2"/> Comparar com IA</>}
                            </RefinedButton>
                            {isAiAnalyzing && (
                                <div className="space-y-4">
                            ...
                            {aiAnalysis && (
                                <Alert variant="default">
                                    <Sparkles className="h-4 w-4" /><AlertTitle className="font-semibold">Análise por IA (Comparativa)</AlertTitle>
