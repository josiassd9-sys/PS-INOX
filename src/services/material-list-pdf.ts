...
type GenerateMaterialListPdfParams = {
  items: MaterialListPdfItem[];
  totalWeight: number;
  totalPrice: number;
};
...
export async function generateMaterialListPdf({
  ...
  totalWeight,
  totalPrice,
}: GenerateMaterialListPdfParams): Promise<'native' | 'web'> {
  ...
  const fileName = `lista_materiais_${stamp}.pdf`;
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(12);
  doc.text('LISTA DE MATERIAIS', 105, 23, { align: 'center' });
  ...
  if (Capacitor.isNativePlatform()) {
    ...
    await Share.share({
      ...
      text: 'PDF da lista de materiais gerado pelo PS INOX',
      url: uri.uri,
