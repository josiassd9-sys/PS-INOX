  ...
  const handleAnalyzeWithAI = async () => {
    ...
    try {
      const response = await fetch("/api/ai-compare", {
        ...
        body: JSON.stringify({
          analysisType: "viga-secundaria",
          context: {
            inputs: { span, balanco1, balanco2, spacing, slabLoad, distributedLoad, pointLoad, pointLoadPosition, steelType, beamScheme, safetyFactor },
            result,
    ...
    <div className="space-y-4">
      <RefinedCard hover="subtle" className="p-0 overflow-hidden">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end [&_label]:truncate [&_label]:leading-tight">
            ...
            <div className="grid grid-cols-2 gap-2 sm:gap-4 md:col-span-2 [&_label]:truncate [&_label]:leading-tight">
              <div className="space-y-2 min-w-0"><LinkedFieldLabel htmlFor="vs-slab-load" label="Carga da Laje (kgf/m²)" linked={fieldLinks.vigaSecundaria.slabLoad} onToggle={(linked) => setFieldLink('vigaSecundaria', 'slabLoad', linked)} source="Resultado da aba Laje" /><Input id="vs-slab-load" type="text" inputMode="decimal" value={slabLoad} onChange={(e) => handleInputChange('slabLoad', e.target.value)} placeholder="Ex: 450" readOnly={fieldLinks.vigaSecundaria.slabLoad} className={fieldLinks.vigaSecundaria.slabLoad ? "bg-muted/70" : ""} /></div>
              <div className="space-y-2 min-w-0"><Label htmlFor="vs-spacing">Espaçamento entre Vigas (m)</Label><Input id="vs-spacing" type="text" inputMode="decimal" value={spacing} onChange={(e) => handleInputChange('spacing', e.target.value)} placeholder="Ex: 1,5" /></div>
            </div>
            <div className="space-y-2 md:col-span-2"><LinkedFieldLabel htmlFor="vs-load" label="Carga Distribuída (kgf/m)" linked={fieldLinks.vigaSecundaria.distributedLoad} onToggle={(linked) => setFieldLink('vigaSecundaria', 'distributedLoad', linked)} source="Carga da laje x espaçamento" /><Input id="vs-load" type="text" inputMode="decimal" value={distributedLoad} onChange={(e) => handleInputChange('distributedLoad', e.target.value)} placeholder="Carga linear" readOnly={fieldLinks.vigaSecundaria.distributedLoad} className={fieldLinks.vigaSecundaria.distributedLoad ? "bg-muted/70" : ""} /></div>
            <div className="grid grid-cols-2 gap-2 sm:gap-4 md:col-span-2 [&_label]:truncate [&_label]:leading-tight">
              <div className="space-y-2 min-w-0"><Label htmlFor="vs-point-load">Carga Pontual (kgf)</Label><Input id="vs-point-load" type="text" inputMode="decimal" value={pointLoad} onChange={e => handleInputChange('pointLoad', e.target.value)} placeholder="Opcional" /></div>
              <div className="space-y-2 min-w-0"><Label htmlFor="vs-point-load-pos">Posição da Carga (m)</Label><Input id="vs-point-load-pos" type="text" inputMode="decimal" value={pointLoadPosition} onChange={e => handleInputChange('pointLoadPosition', e.target.value)} placeholder="Dist. do apoio esquerdo" /></div>
            </div>
           ...
           <RefinedButton type="button" onClick={handleCalculate} className="w-full md:w-auto">Calcular Viga Secundária</RefinedButton>
